...
async def test_pair_tool_returns_qr_png_and_qr_unicode(tmp_path, monkeypatch):
    ...
    Pinned to local mode (``TOTALRECLAW_PAIR_MODE=local``) so this test
    can run without a live relay. The QR contract is mode-agnostic — the
    relay path uses the same ``encode_png`` / ``encode_unicode`` helpers.
    """
    ...
    # Pin to local mode — rc.10 default is relay but this test must not
    # hit the network.
...
async def test_pair_tool_payload_never_contains_phrase_material(tmp_path, monkeypatch):
    ...
    Pinned to local mode for offline testability — same payload shape
    applies to the relay path (see test_pair_remote_client).
    """
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("TOTALRECLAW_PAIR_BIND_HOST", "127.0.0.1")
    monkeypatch.setenv("TOTALRECLAW_PAIR_BIND_PORT", "0")
