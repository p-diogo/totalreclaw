...
- **[#581] `derived-bundle-v1` pair-completion branch (Option E Phase 2, P2-10 — the cred-12 leaf).** `pair/remote_client.py::await_phrase_upload` now dispatches on the pair envelope's `payload_type` field: `derived-bundle-v1` decrypts the inner plaintext as a bundle (`totalreclaw.bundle.parse_bundle_v1` — every §4.7 invariant checked; a malformed bundle nacks and is never persisted), and the existing `legacy-mnemonic` (or absent) path is unchanged for every other call site. `pair/completion_sidecar.py` adds `_complete_pairing_bundle`, wired via the new optional `complete_pairing_bundle` parameter — wraps `{vault, signing}` via the keychain (or falls back to an in-file plaintext v2 write, matching P2-9's auto-migration policy), writes `credentials.json`, and activates the client via `AgentState.configure_from_bundle`. An additional host-side check rejects a bundle whose `account.chain_id != 100` (single-chain Gnosis policy) before anything is written. **Not yet reachable via live traffic**: the relay does not forward a `payload_type` field on the pair envelope yet (P2-0 pre-flight finding; plumbing tracked separately as P2-11) — this leaf is built and tested against a fixture loopback relay (`tests/test_pair_completion_bundle.py`, 9 tests: happy path lands a real keychain-wrapped v2 credential store, malformed-hex/address-mismatch/unknown-kind all reject cleanly with nothing persisted, wrong-chain rejects before any write, the legacy payload_type still completes, an unknown future payload_type nacks loudly), per the implementation spec's explicit instruction to build this leaf against a fixture provisioning half rather than live SPA/relay traffic.
- **[#581] `derived-bundle-v1` credential bundle — Hermes-side migration (Option E Phase 2, P2-5 through P2-9).** A host can now be configured from a `derived-bundle-v1` bundle (the four HKDF-derived vault keys + an owner-EOA signing key) instead of the BIP-39 recovery phrase — the mnemonic and the 64-byte seed never exist on a bundle-configured process. `totalreclaw.bundle` (new) — `DerivedBundle`/`VaultKeys`/`SigningMaterial`/`AccountRef` frozen dataclasses, `derive_bundle_from_mnemonic`/`parse_bundle_v1`/`validate_bundle_v1` delegating entirely to `totalreclaw_core`; `repr()`/`str()` redact every secret field. `credentials_wrap.py`: `wrap_bundle`/`unwrap_bundle` keyed on `__keychain__:v2:<smart_account>` (distinct namespace from v1's EOA-keyed marker — the two coexist during migration), plus `delete_secret` (new primitive, needed to drop the v1 entry post-migration). `TotalReclaw.from_bundle(bundle, ...)` constructs a client with `self._mnemonic = None`, `wallet_address`/`chain_id` resolved immediately from the bundle (zero network calls) — byte-identical vault-keyed crypto output to a mnemonic-configured client on the same vault (verified: `auth_key_hash`, cross-decryptable ciphertext, content fingerprints, all 20 LSH buckets). `register()` raises on a bundle-configured client; the internal auto-registration check silently skips it instead (the vault is already registered by the provisioning origin). `AgentState._try_auto_configure` now implements the full four-state credential precedence (env phrase > credential-provider payload, sniffed by `version` > legacy plaintext/v1-marker), with an unrecognised `version` raising loudly rather than ever silently downgrading to legacy handling. `hermes/auto_migrate.py` (new — the cred-10 leaf) runs a transparent, idempotent, lock-guarded local migration on every plugin load: resolves the existing phrase, derives the bundle, wraps it (or falls back to an in-file plaintext v2 write on a keychain-less host), atomically replaces `credentials.json` (restoring from `.bak` on any write failure), and only after the new file reads back correctly zeroes the local mnemonic reference and drops the v1 keychain entry. `credentials.json.bak` is retained unconditionally this release — the switch to immediate destruction is a deliberate, evidence-gated one-line edit in a later release (tracked #579; do not automate). Opt-out: `TOTALRECLAW_NO_AUTO_MIGRATE`. 81 new tests across `test_bundle.py`, `test_client_bundle_mode.py`, `test_state.py`, `test_auto_migrate.py`, plus extensions to `test_credentials_wrap.py` — including a load-bearing offline test proving a fact encrypted before migration decrypts correctly through the migrated, bundle-configured client. Cross-language parity (core + WASM + PyO3 bindings, the `bundle.rs` derivation itself) shipped separately in #587.
