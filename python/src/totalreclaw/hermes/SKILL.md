...
- **One-shot / short-lived sessions** (e.g. `hermes chat -q`): a large import runs in a BACKGROUND task that dies when the process exits, so nothing gets stored. In those contexts, drive `totalreclaw_import_batch` **synchronously in a loop** (increasing `offset` until `is_complete`) instead of relying on the background path — or keep a long-lived gateway/daemon running.
...
- **Privacy disclosure (mandatory, ChatGPT/Gemini/Claude)**: the tool returns `disclosure_required` + a one-time `disclosure_token` before any extraction — relay its message to the user **verbatim** (it names the exact LLM provider that will read their past conversations in cleartext; NEVER compose your own disclosure wording), get explicit consent, then re-call with `disclosure_confirmed=true` **and** the `disclosure_token`. Consent without the token is rejected — you cannot pre-consent on the user's behalf. If the user declines, stop — nothing is processed. Resuming an import never re-prompts.
