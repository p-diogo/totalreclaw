/**
 * OpenMemory Skill - Main Skill Class
 *
 * The primary integration point for OpenMemory with OpenClaw.
...
/**
 * Adapter to make OpenMemory client available as a vector store
 */
class OpenMemoryVectorStoreAdapter implements VectorStoreClient {
  private client: OpenMemory;

  constructor(client: OpenMemory) {
    this.client = client;
  }
  ...
  async getEmbedding(text: string): Promise<number[]> {
    // The OpenMemory client handles embedding internally
...
// ============================================================================
// OpenMemorySkill Class
// ============================================================================

/**
 * Main OpenMemory Skill Class
 *
 * Integrates OpenMemory with OpenClaw through lifecycle hooks and tools.
 */
export class OpenMemorySkill {
  private config: OpenMemorySkillConfig;
  private client: OpenMemory | null = null;
  private reranker: CrossEncoderReranker;
  private extractor: FactExtractor | null = null;
  private state: SkillState;
  private llmAdapter: OpenClawLLMAdapter | null = null;
  private vectorStoreAdapter: OpenMemoryVectorStoreAdapter | null = null;

  /**
   * Create a new OpenMemorySkill instance
   *
   * @param config - Skill configuration (partial, will be merged with defaults)
   */
  constructor(config: Partial<OpenMemorySkillConfig> = {}) {
   ...
   * Initializes the OpenMemory client and loads the reranker model.
   *
    ...
    try {
      ...
      if (this.config.masterPassword) {
        if (this.config.userId && this.config.salt) {
          ...
          console.log(`[OpenMemory] Registered new user: ${userId}`);
        }
      }

      // Initialize reranker (async, non-blocking)
      this.reranker.load(this.config.rerankerModel).catch(err => {
        console.warn('[OpenMemory] Failed to load reranker model, using fallback:', err);
      });

      // Set up LLM adapter if provided
      if (llmCompleteFn) {
        this.llmAdapter = new OpenClawLLMAdapter(llmCompleteFn);
        this.vectorStoreAdapter = new OpenMemoryVectorStoreAdapter(this.client);
      ...
      console.error('[OpenMemory] Initialization failed:', message);
  ...
  async onBeforeAgentStart(context: OpenClawContext): Promise<BeforeAgentStartResult> {
    ...
    try {
      ...
      console.error('[OpenMemory] onBeforeAgentStart failed:', error);
  ...
  async onAgentEnd(context: OpenClawContext): Promise<AgentEndResult> {
    ...
    try {
      ...
      console.error('[OpenMemory] onAgentEnd failed:', error);
  ...
  async onPreCompaction(context: OpenClawContext): Promise<PreCompactionResult> {
    ...
    try {
      ...
      console.error('[OpenMemory] onPreCompaction failed:', error);
  ...
  /**
   * Get the underlying OpenMemory client
   */
  getClient(): OpenMemory | null {
    return this.client;
  }
  ...
  private ensureInitialized(): void {
    if (!this.state.isInitialized || !this.client) {
      throw new OpenMemoryError(
        ...
        'OpenMemory skill not initialized. Call init() first.'
      );
    }
  }
  ...
  private async extractFacts(
    ...
    if (!this.extractor) {
      console.warn('[OpenMemory] No extractor configured, skipping fact extraction');
      return {
        facts: [],
        rawResponse: '',
        processingTimeMs: 0,
...
/**
 * Create an OpenMemory skill instance with default configuration
 */
export function createOpenMemorySkill(
  config?: Partial<OpenMemorySkillConfig>
): OpenMemorySkill {
  return new OpenMemorySkill(config);
}
...
export default OpenMemorySkill;
