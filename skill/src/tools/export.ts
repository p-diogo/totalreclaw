/**
 * OpenMemory Skill - Export Tool
 *
 * Tool for exporting memories from OpenMemory.
 * This is the openmemory_export tool implementation.
 *
...
import type { OpenMemory, ExportedData, Fact } from '@openmemory/client';
...
/**
 * Export all memories from OpenMemory
 *
...
function formatAsMarkdown(data: ExportedData): string {
  ...
  lines.push('---');
  lines.push('');
  lines.push('_Generated by OpenMemory Skill_');
  lines.push('');
 ...
 */
export function createExportTool(client: OpenMemory): (params: ExportToolParams) => Promise<ExportToolResult> {
