...
I hope this captures the key points.`;
  ...
  const logs: string[] = [];
  // The bracket-scan recovery path logs at DEBUG level (both compaction and
  // non-compaction parsers were deliberately lowered from info→debug to keep
  // this benign, common fallback out of the info stream). Capture debug too, or
  // the "recovery logged" assertion can never observe it.
  const mockLogger = {
