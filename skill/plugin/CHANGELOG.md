...
Skips 3.4.3, which was drafted but abandoned. 3.4.3's premise was "this client cannot pin, so make the prohibition unmissable by moving it into the `memory_save` tool description." #563 makes the opposite true — the client can now pin / unpin / retype / set-scope via the `tr` CLI — so shipping 3.4.3 would publish a claim we immediately reverse. Only one commit from the 3.4.3 line survives (the bare-`tr` pairing-banner fix, below); the "cannot pin" tool-description change is dropped because #563 supersedes it and would have conflicted.

### Added

- **[#573] Curation is now a first-class agent tool surface: `memory_pin` / `memory_unpin` / `memory_retype` / `memory_set_scope`.** The CLI subcommands (#563, below) closed the *capability* gap, but the handoff's measured evidence showed shell-out-shaped surfaces degrade: with no tool available, an agent told to "pin that" improvised — one `memory_save` at importance 10, described as "as close as pinning gets" — when importance affects RANKING while pin protects against AUTO-SUPERSESSION (different guarantees); on an earlier build the same ask produced outright confabulation ("Done — pinned with importance 10…", all invented). Four in-process tools now register alongside `memory_search`/`memory_get`/`memory_save` via a `curate` closure (`memory/curation-runtime.ts` mirrors `native-store.ts`): argument shape follows MCP parity (`memory_id` canonical + `fact_id` back-compat alias; `reason` pin-only; `new_type` retype-only; `scope` set-scope-only), normalised into the shared `runCurationOp` dispatch extracted to the pure `memory/curation-op.ts` (importing it from `cli/tr-cli.ts` would pull the whole CLI bin graph into the agent-tool path). Supersession logic is reused, not reimplemented; `sourceAgent: 'openclaw-plugin-curated'` distinguishes agent-tool curation from CLI (`'tr-cli'`) in on-chain provenance. **The tool description is the behavioural control surface** — the measured §6 lesson: a prohibition that failed to hold as SKILL.md prose has held as tool-description prose since 3.4.0 — so each description names the on-chain supersession semantics (NEW `fact_id` + `tx_hash`, original id no longer active, look the id up via `memory_search` first, report results truthfully) and states that storing at high importance is NOT a substitute. `memory_save`'s description and SKILL.md are repointed from the CLI to the tools (subcommands documented as manual/debug). The **`contracts.tools` manifest lockstep** — OpenClaw *silently drops* a `registerTool` whose name is not declared there, shipping the feature dead with green tests — is extended to all four names and individually asserted in `manifest-shape.test.ts` 1f, which previously hard-coded only the three original names (a half-revert would have passed the old guard silently). Validated end-to-end: 99/99 unit tests (new `curate-tool.test.ts` + `register-native.test.ts` contract 6), a staging E2E 17/17 (every op verified as a real on-chain supersession on the isolated staging DataEdge: old id tombstoned, replacement active, tx hashes), and a live agent behavioural test — asked to "remember X, then pin that memory", the agent called `memory_save` → `memory_search` (id lookup) → `memory_pin` (real supersession, tx `0xe7dcd2e4…`) instead of confabulating an importance-10 store. ([#573](https://github.com/p-diogo/totalreclaw/issues/573))

- **[#563] Curation — pin / unpin / retype / set-scope — is now available in this client, via the `tr` CLI.** The pure operations (`executePinOperation` / `executeRetype` / `executeSetScope`) and their unit tests already lived in `memory/` — the MCP server and NanoClaw wired them, the OpenClaw plugin never did. So an agent told "pin that" had no real path: it stored an ordinary `memory_save` and reported a successful pin, leaving the user believing a fact was protected from auto-supersession when it was nothing of the sort — the same class of silent data-integrity loss as the #551 shell-`tr` failure. `cli/tr-cli.ts` gains `tr pin` / `tr unpin` / `tr retype` / `tr set_scope` subcommands sharing a `cmdCuration` core that delegates to those existing operations (supersession logic is reused, not reimplemented); arg parsing and the `execute*` delegation are exported (`parseCurationCliArgs`, `runCurationOp`, `buildCurationDeps`) so the success / bad-args / idempotent / not-found paths are unit-testable in-process, mirroring the MCP server's `buildPinDepsFromState` deps wiring. Each op is an on-chain supersession — the old fact is tombstoned and a fresh one with the changed field is written, so the response carries a **new** `fact_id` + `tx_hash`; the original id is no longer active. These are CLI commands the user (or agent, via the resolved `TR_CLI` below) runs; the `memory_save` tool description points at them. *(Written when this entry was CLI-only — later in the same release, [#573] above added the four in-process agent tools that supersede the CLI as the agent-facing path; the subcommands remain as manual/debug.)* ([#563](https://github.com/p-diogo/totalreclaw/issues/563))

  **Why this resolves #563 rather than re-prosing it.** 3.4.2's truth-fix removed the false SKILL.md advertisement of these commands and replaced it with a prohibition. Re-tested on 3.4.2, the prohibition did **not** hold — a direct phrasing ("pin the fact that I bought LLY today so it never gets overwritten") still produced a confabulated pin (`memory_save` ×1, "Done — pinned with importance 10…", all invented), while a question-shaped phrasing declined correctly, so the behaviour was phrasing-dependent rather than controlled. The lesson that survives is the one 3.4.3 was built around: **SKILL.md prose is advisory — consulted sometimes, not a control surface.** 3.4.4 doesn't try harder prose; it makes the capability real in code and puts the "here is the real path / do not fake it" instruction where the model sees it every turn (the `memory_save` tool description) alongside commands that actually run.

### Fixed

- **The `TR_CLI` glob in SKILL.md resolved to empty for a real npm install.** It listed only `~/.openclaw/npm/node_modules/@totalreclaw/totalreclaw/…` and `~/.openclaw/extensions/totalreclaw/…` — neither of which `openclaw plugins install` creates. npm installs unpack to `~/.openclaw/npm/projects/<slug-hash>/node_modules/@totalreclaw/totalreclaw/`, so `TR_CLI` came back empty and every `node "$TR_CLI" …` instruction failed. A real agent hit exactly this in the 3.4.2 QA and reported it could not find the CLI ("may not be in PATH under that name"). Pre-existing bug, but it would have made the entire curation feature above unusable via the documented path the moment it shipped. The `projects/*totalreclaw*/` glob is now listed first (older/dev layouts kept as fallbacks); verified live on OpenClaw 2026.6.8 that it resolves and `status --json` returns real JSON. Regression guard in `skill-md-native.test.ts`.
...
- **[#545] Graceful fail on a keychain-wrapped `credentials.json`, instead of a hard crash.** All clients share `~/.totalreclaw/credentials.json`. Once a co-installed Hermes client wraps the recovery phrase in the OS keychain (#546) it leaves a non-secret marker (`__keychain__:v1:<eoa>`) in the mnemonic field. `initialize()` previously read that marker as if it were the phrase and handed it to `deriveKeys()`, which threw an unhandled `invalid mnemonic: expected 12 or 24 words, got 1` — a hard crash. Safety was never at risk (the marker fails BIP-39 validation loudly everywhere, so no wrong-wallet derivation was ever possible); this was an availability/UX break only. `initialize()` (and the `attemptHotReload()` re-entry path) now detect the marker up front via the new `skill/plugin/keychain-marker.ts` helper and fail soft — `needsSetup = true` plus a clear, actionable log message pointing at `TOTALRECLAW_RECOVERY_PHRASE` or `TOTALRECLAW_NO_KEYCHAIN=1` on the Hermes side — instead of crashing. New `keychain-marker.test.ts`.

  **Fix-round addition:** review caught that `cli/tr-cli.ts` `buildContext()` — the shared credential-load path behind `tr remember` / `tr forget` / `tr export` — had the identical unguarded read→derive path and reproduced the exact same crash. The credential-read+marker-guard prelude is now extracted into an exported `resolveCliMnemonicOrDie()`, which `die()`s with the same actionable message on a marker before `setRecoveryPhraseOverride()`/`deriveKeys()` are ever reached. New wiring-level `cli/tr-cli-keychain-marker.test.ts` drives that function in-process (confirmed to fail red without the guard, green with it) — the helper-only unit tests alone had missed this second call site. `main()`'s auto-invocation is now gated on an "is this the direct entry script" check (`import.meta.url` vs `process.argv[1]`) so importing `resolveCliMnemonicOrDie()` for tests doesn't also run the live CLI as an import side effect — production behavior (`tr <cmd>` always runs as the direct entry script) is unchanged. ([#545](https://github.com/p-diogo/totalreclaw/issues/545))

  **CI fix:** the first cut of the wiring test spawned `tr-cli.ts` as a subprocess (`node:child_process`), which the plugin's install-scanner CI gate (`scripts/check-scanner.mjs`, `SHELL_EXEC_PATTERN`) rejected — the rule is a bare pattern match on ANY file in the tree, comments included, no exceptions (mirroring OpenClaw's own installer refusal of shell-out-capable plugins). Rewritten to stub `process.exit`/`process.stderr.write` instead of shelling out, per the `resolveCliMnemonicOrDie()` extraction above.

- **SKILL.md no longer advertises curation commands this client does not have.** It listed `tr pin` / `tr unpin` / `tr retype` / `tr set_scope` as available; all four return `tr: Unknown command`, and there is no pin tool in the manifest either — they exist only in the MCP server and NanoClaw. An agent asked to "pin that" therefore stored an ordinary memory and reported a successful pin, leaving the user believing a fact was protected from auto-supersession when it was not. Same class as the #551 shell-`tr` silent data loss: a confident success report for an operation that never happened. Caught in the 3.4.2-rc.1 fresh-install QA on OpenClaw 2026.6.8. SKILL.md now states plainly that this client cannot pin, unpin, retype or re-scope, and forbids substituting a store for a pin; the plugin README's "natural-language overrides" claim is corrected to match. Implementing the commands for real is tracked in [#563](https://github.com/p-diogo/totalreclaw/issues/563). ([#563](https://github.com/p-diogo/totalreclaw/issues/563))

- **Second delivery path for the agent-facing SKILL.md.** `openclaw.plugin.json` gained `"skills": ["."]`, so OpenClaw also links the bundled `SKILL.md` into `~/.openclaw/plugin-skills/` at gateway start, and `openclaw skills list` reports `✓ ready 🧠 totalreclaw`. Verified on OpenClaw 2026.6.8 against the real published artifact, and guarded by new assertions in `manifest-shape.test.ts`.

  **Correction (2026-08-01):** the changelog and PR for this originally claimed the plugin had *never* delivered its own SKILL.md and that the prose only ever arrived via a separate `openclaw skills install` from ClawHub. **That was wrong.** `setup/skill-register.ts` has always mirrored `SKILL.md` + `skill.json` into `~/.openclaw/workspace/skills/totalreclaw/` at `register()` time, precisely so a single `openclaw plugins install` suffices. The mistaken claim came from grepping a stale branch on which that file was absent, without checking `main`. The manifest field is therefore **redundant, not a fix** — a harmless second path, kept because it makes the skill visible to `openclaw skills list` and because the tests around it are worth having. Nothing was broken before it, and adding it fixed no user-visible bug.
