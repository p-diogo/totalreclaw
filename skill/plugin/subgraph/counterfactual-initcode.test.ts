 ...
 * Accounts (the AA24 ship-stopper, 2026-06-27; PR #391 hand-integration).
 *
 ...
 *
 * Scenario 2 then proves `reassertInitCodeAfterSponsorship` restores the
 * initCode when a sponsor response clobbers it with nulls.
 *
...
async function mockFetch(input: string | URL | Request, init?: RequestInit): Promise<Response> {
  ...
  // Chain-RPC methods (eth_getCode, eth_call for nonce) hit the rpcUrl;
...
// ---------------------------------------------------------------------------
// Scenario 1: counterfactual sender → initCode present + signed after sponsor.
// ---------------------------------------------------------------------------
...
const config: SubgraphStoreConfig = {
  ...
  // Pin the rpcUrl so the test never consults plugin CONFIG / env defaults —
  // the mock fetch is URL-agnostic anyway.
...
try {
  ...
  // sponsored gas limits all present) and recover — MUST match the owner.
  // If the plugin signed a DIFFERENT object (e.g. before sponsorship, or
  // before adding factory), this check fails with a recovered-address
  // mismatch — the exact signature-invalid condition the EntryPoint
  // surfaces as AA24.
...
// restored before signing (reassertInitCodeAfterSponsorship's whole purpose).
// Without the guard, the sponsored UserOp would ship WITHOUT initCode → AA24.
// ---------------------------------------------------------------------------

{
  ...
  // Use a DIFFERENT counterfactual sender than scenario 1 — hermetic per
  // scenario (getInitCode re-checks eth_getCode every call since the AA10
  // fix, but a fresh address keeps the scenarios independent regardless).
...
assert.ok(true); // keep the assert import load-bearing for tsx strip
if (failed > 0) {
