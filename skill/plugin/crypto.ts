 ...
 *
 * Lenient: accepts phrases where all words are in the wordlist even if
 * the BIP-39 checksum is invalid (LLMs sometimes generate such phrases).
 */
 ...
 * and HKDF key separation. Uses the lenient variant for phrases where
 * all words are valid but the checksum fails.
 */
function deriveKeysFromMnemonic(
  mnemonic: string,
): { authKey: Buffer; encryptionKey: Buffer; dedupKey: Buffer; salt: Buffer } {
  const trimmed = mnemonic.trim();
  const isStrict = validateMnemonic(trimmed, wordlist);
  const result = isStrict
    ? wasm.deriveKeysFromMnemonic(trimmed)
    : wasm.deriveKeysFromMnemonicLenient(trimmed);
  return {
