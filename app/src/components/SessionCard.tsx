...
interface Props {
  group: SessionGroup;
  /** Navigate to the full session page (List → page drill). */
  href?: string;
  /** Open the shared side-panel instead of navigating (redesign default). When
   *  set, takes precedence over `href`. */
  onOpen?: () => void;
  style?: CSSProperties;
  /** When set, entity chips drill into that entity (panel) or filter. */
...
export function SessionCard({ group, href, onOpen, style, onEntityClick }: Props) {
  ...
  const bodyClass =
    "block w-full rounded-card text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-clay focus-visible:ring-offset-2";
  const body = (
    <>
  ...
  return (
    <article
      className="animate-fade-up flex h-full flex-col rounded-card bg-surface p-5 shadow-soft transition duration-200 ease-keeper hover:-translate-y-0.5 hover:shadow-raised"
      style={style}
    >
      {onOpen ? (
        <button type="button" onClick={onOpen} className={bodyClass}>
          {body}
        </button>
      ) : (
