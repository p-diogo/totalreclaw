#!/usr/bin/env python3
"""
LSH Parameter Validation on Combined WhatsApp + Slack Dataset

Re-validates the LSH parameters on a larger combined dataset (8,727 embeddings)
to verify they still work at scale.

Validated parameters to test:
- n_bits=64, n_tables=12, candidate_pool=2000 (best from WhatsApp)
- n_bits=64, n_tables=12, candidate_pool=1500 (reduced overhead test)
"""
...
WHATSAPP_EMBEDDINGS_PATH = Path("/Users/pdiogo/Documents/code/totalreclaw/testbed/v2-realworld-data/processed/embeddings.npy")
SLACK_EMBEDDINGS_PATH = Path("/Users/pdiogo/Documents/code/totalreclaw/testbed/v2-realworld-data/processed/slack_embeddings.npy")
COMBINED_OUTPUT_PATH = Path("/Users/pdiogo/Documents/code/totalreclaw/testbed/v2-realworld-data/processed/combined_embeddings.npy")
RESULTS_PATH = Path("/Users/pdiogo/Documents/code/totalreclaw/testbed/validation/lsh_results.json")

# Validated parameters from WhatsApp experiments to test on combined data
# Format: (n_bits_per_table, n_tables, n_candidates_pool)
# Round 2: Focus on finding sweet spot around 93% recall
PARAM_COMBINATIONS = [
    # Sweet spot hunting with n_bits=64, n_tables=12
    (64, 12, 2800),
    (64, 12, 2900),
    (64, 12, 3000),
    (64, 12, 3100),
    # Also test with n_tables=16 for comparison
    (64, 16, 2800),
    (64, 16, 2900),
    (64, 16, 3000),
    # Best configs from round 1 for context
    (64, 12, 3500),
    (64, 12, 4000),
]

N_QUERIES = 200  # More queries for larger dataset
TOP_K_TRUE = 250
RANDOM_SEED = 42
...
def load_and_combine_embeddings() -> np.ndarray:
    """Load both embedding sets and combine them."""
    print(f"Loading WhatsApp embeddings from {WHATSAPP_EMBEDDINGS_PATH}")
    wa = np.load(WHATSAPP_EMBEDDINGS_PATH)
    print(f"  Shape: {wa.shape}, dtype: {wa.dtype}")

    print(f"Loading Slack embeddings from {SLACK_EMBEDDINGS_PATH}")
    slack = np.load(SLACK_EMBEDDINGS_PATH)
    print(f"  Shape: {slack.shape}, dtype: {slack.dtype}")

    # Combine
    combined = np.vstack([wa, slack])
    ...
    # Save for future use
    np.save(COMBINED_OUTPUT_PATH, combined)
...
def main():
    print("=" * 80)
    print("TotalReclaw LSH Parameter Validation - COMBINED Dataset")
    print("=" * 80)

    embeddings = load_and_combine_embeddings()
    ...
    print(f"\nDataset: {n} embeddings ({n} x {embeddings.shape[1]})")
    print(f"Running {total} experiments with {len(query_indices)} queries each...")
    print("-" * 80)
    ...
    for i, (bits, tables, cands) in enumerate(PARAM_COMBINATIONS):
        ...
        print(f"  Recall: {r.mean_recall:.3f} (p5={r.p5_recall:.3f}, p95={r.p95_recall:.3f}) | Cands: {r.mean_candidates_returned:.0f} | Query: {r.mean_query_time_ms:.2f}ms | Target: {ok}")

    # Analyze
    sorted_r = sorted(results, key=lambda x: -x.mean_recall)

    # Select optimal config: meets target with minimal candidate pool
    # (prioritize efficiency over maximum recall)
    meeting_target = [r for r in sorted_r if r.meets_target]
    if meeting_target:
        # Sort by candidate pool (ascending) to get most efficient
        meeting_target_sorted = sorted(meeting_target, key=lambda x: x.n_candidates)
        best = meeting_target_sorted[0]
    else:
        best = sorted_r[0]
    ...
    if best.meets_target:
        rec = f"""RECOMMENDATION: LSH parameters validated on combined dataset.
...
- Candidates returned: ~{best.mean_candidates_returned:.0f}

NOTES:
- Parameters scaled from WhatsApp-only validation (2000 -> {best.n_candidates} candidates)
- The larger, more diverse Slack dataset requires larger candidate pool
- Consider adaptive candidate pool: min({best.n_candidates}, n_memories * 0.35)

LSH IS VIABLE for TotalReclaw v0.3 at scale (8,727 embeddings tested)."""
    else:
        gap = 0.93 - best.mean_recall
        rec = f"""WARNING: LSH does not achieve 93% recall target on combined dataset.
...
NEED TO INVESTIGATE:
1. Try larger candidate_pool (2500-3000)
2. Try more tables (16-20)
3. Consider hybrid approach with random sampling"""
    ...
    # Load existing results and add combined_data section
    existing_results = {}
    ...
    existing_results['combined_data'] = {
        'metadata': {
            'whatsapp_embeddings_path': str(WHATSAPP_EMBEDDINGS_PATH),
            'slack_embeddings_path': str(SLACK_EMBEDDINGS_PATH),
            'combined_embeddings_path': str(COMBINED_OUTPUT_PATH),
            'n_whatsapp': 1162,
            'n_slack': 7565,
            'n_total': 8727,
            'n_queries': N_QUERIES,
    ...
    # Compare with WhatsApp results
    print("\n" + "=" * 80)
    print("COMPARISON: WhatsApp (1,162) vs Combined (8,727)")
    print("=" * 80)

    # Find matching result from WhatsApp experiments
    wa_result = None
    if 'best_params' in existing_results:
        wa_result = existing_results['best_params']

    if wa_result:
        print(f"{'Metric':<25} {'WhatsApp':>15} {'Combined':>15} {'Delta':>10}")
        print("-" * 65)
        print(f"{'Mean Recall':<25} {wa_result['mean_recall']*100:>14.1f}% {best.mean_recall*100:>14.1f}% {(best.mean_recall - wa_result['mean_recall'])*100:>+9.1f}%")
        print(f"{'P5 Recall':<25} {wa_result['p5_recall']*100:>14.1f}% {best.p5_recall*100:>14.1f}% {(best.p5_recall - wa_result['p5_recall'])*100:>+9.1f}%")
        print(f"{'Query Latency (ms)':<25} {wa_result['mean_query_time_ms']:>15.2f} {best.mean_query_time_ms:>15.2f} {(best.mean_query_time_ms - wa_result['mean_query_time_ms']):>+10.2f}")
        print(f"{'Candidates Returned':<25} {wa_result['mean_candidates_returned']:>15.0f} {best.mean_candidates_returned:>15.0f} {(best.mean_candidates_returned - wa_result['mean_candidates_returned']):>+10.0f}")
