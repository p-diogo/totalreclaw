...
## totalreclaw (Python / Hermes) 2.5.2 — patch release (2026-09-26)

- **Fixed: relay read denials are no longer silent** ([#662](https://github.com/p-diogo/totalreclaw/issues/662)). When the relay refused memory reads (a monthly read-quota denial or a rate limit), Hermes used to treat it as "no memories", so recall quietly returned nothing, and every recall kept re-sending queries into the denial. Hermes now tells you that memory lookups are paused and why. It stops a recall at the first denied query, pauses reads until the relay's retry time (re-checking every 15 minutes), and says once when lookups work again. Writes are unaffected. `totalreclaw_export` no longer returns a silent partial export on relay errors. Relay side, shipped the same day: reads are now counted rather than capped (observe-only), and wallet addresses are normalized, so one wallet can no longer end up with split quota rows.

## totalreclaw (Python / Hermes) 2.5.1 — patch release (2026-09-16)

- **Fixed: the Hermes memory-provider sidecar is now a full self-healing plugin file** instead of a bare 3-line re-export stub ([#638](https://github.com/p-diogo/totalreclaw/issues/638)). The old stub silently killed the plugin whenever Hermes regenerated its managed Python runtime (the recreated venv lost the pip-installed `totalreclaw` package and the plugin vanished with **no error anywhere** — no tools, no hooks, no memory provider). The new sidecar attempts one bounded reinstall into the running interpreter's venv on `ImportError`; failing that it loads in DEGRADED mode registering a `totalreclaw_status` tool that reports the missing package and the exact fix command. Boxes already carrying the old stub heal with `totalreclaw hermes install-memory-provider --force` or a re-pair; credentials and vault data are never affected. Validated live end-to-end on a production bot: install without shadow-version drift, sidecar relaid, account restored without re-pairing (739 memories intact), explicit + autonomous fact storage (739 → 746), and a `totalreclaw_recall` round-trip quoting vault entries with fact IDs.
- **SKILL.md: `session_search` framed as a fallback only** ([#637](https://github.com/p-diogo/totalreclaw/pull/637)) — Hermes' host-level `session_search` tool reads local session transcripts with limited retention; it must never substitute for vault recall.
- **Pinned `transformers>=4.40,<5`** — transformers 5.x resolves on newer Pythons and breaks the Harrier embedder's model load (rope-handling change inside `transformers.modeling_rope_utils`). Pre-existing unpinned range; the ceiling restores the long-tested 4.x path.
