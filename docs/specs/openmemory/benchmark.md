<!--
Product: OpenMemory
Formerly: tech specs/v0.3 (grok)/TS v0.3: OpenMemory Benchmark Harness (OMBH).md
Version: 1.1
Last updated: 2026-02-24
-->

# Technical Specification: OpenMemory Benchmark Harness (OMBH) v1.1 — 3-Skill Edition

**Version:** 1.1 (Complete & Self-Contained for Coding Agent)  
**Date:** February 20, 2026  
**Author:** Grok (on behalf of the OpenMemory team)  
**Target Audience:** Any coding agent / developer with **zero prior context** on OpenMemory, OpenClaw, or the project.  
...
1. **OpenMemory E2EE (Crypto/LSH)** — Your new ClawHub skill + OpenMemory server (using the LSH + encrypted-embeddings spec). This is the privacy champion we are proving.
...
- The new private solution (OpenMemory)
...
```
OpenMemory E2EE does not need a full OpenClaw instance — it uses the pure client library (in-process).
...
- `ombh/backends/openmemory_e2ee.py` — Uses your new OpenMemory Python client library (LSH version).
...
Each adapter must respect the exact same extraction triggers and config values as defined in the OpenMemory Skill spec.
...
```python -m ombh.run \
  --systems openmemory_e2ee,openclaw_qmd,openclaw_mem0 \
...
Privacy column: 100 % for OpenMemory, 0 % for the others.


## 8. Verification Suite (Run Before Any A/B)
Automated checks (run via python -m ombh.verify):

Correctness: Force “remember I hate pineapple” → ask 20 sessions later → must retrieve with high score in OpenMemory.
Forgetting: Simulate 90 days decay → verify low-importance facts evicted in OpenMemory.
Portability: Export/import OpenMemory with password → re-test queries (100 % match).
Consistency: Run same conversation 3× → variance <2 %.
Privacy leak test: Attempt simple inversion on OpenMemory blind indices/embeddings → must score 0 usable info.
