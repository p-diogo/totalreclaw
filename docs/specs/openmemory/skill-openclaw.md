<!--
Product: OpenMemory
Formerly: tech specs/v0.3 (grok)/TS v0.3: OpenMemory Skill for OpenClaw.md
Version: 0.3 (1.0)
Last updated: 2026-02-24
-->

# Technical Specification: OpenMemory Skill for OpenClaw

**Version:** 1.0 (Draft for Coding Agent)
**Date:** February 20, 2026
**Author:** Grok (on behalf of the OpenMemory team)
**Target:** OpenClaw v0.x (GitHub: openclaw/openclaw)
**Primary Storage:** OpenMemory E2EE server (zero-knowledge, password-based restore)
**Philosophy Alignment:** Keep OpenClaw's Markdown-first "source of truth" for human readability/auditability while adding structured E2EE facts + graph layer behind the scenes. The Markdown files remain writable/editable by the agent and user; OpenMemory acts as the private, portable, searchable backend.
...
All data is defined in strict TypeScript interfaces (and equivalent JSON Schema for validation). These are used both in the OpenClaw plugin and in the OpenMemory client library.

```ts
// Core Memory Fact (stored as one encrypted blob per fact)
interface OpenMemoryFact {
...
### Storage on OpenMemory Server (per user, under one namespace)
...
3. If any item has `importance ≥ 7` → immediate `store_openmemory_batch`.
...
- When user (or agent) edits a Markdown file in `memory/` dir → watcher fires → re-extract facts from diff → update OpenMemory.
...
**Import Behavior**:
- Validates data structure against OpenMemoryFact schema
...
2. **LLM Fact Extraction Call** (one structured output prompt – see exact prompt in appendix if needed, but use the pre-compaction one below).
   ...
   - Output: array of `OpenMemoryFact` (JSON mode + strict schema validation).
3. **Deduplication**:
...
6. **For each fact + graph delta**:
   - Generate embedding (local or via OpenMemory client).
   - Generate blind indices.
   - AES-256-GCM encrypt (master password derived key using Argon2id).
   - Upload batch to OpenMemory server (or queue if offline).
...
4. Deduplicate against existing memories (call recall_openmemory with summary of current session).
5. Call store_openmemory_batch(facts) – do NOT write to Markdown yet.
6. Then append a human-readable summary to memory/daily/YYYY-MM-DD.md (append only).
```

### Retrieval Pipeline (E2EE two-pass, unchanged from OpenMemory v0.2)
...
| `src/tools/builtin.ts` | Register `recall_openmemory` and `remember_fact` |
...
memory:
  openmemory:
    enabled: true
    serverUrl: "https://api.openmemory.dev"
...
`openclaw-openmemory-plugin` (NPM package) that registers hooks via OpenClaw's plugin system.
...
|------------|-------------|---------|
| `memory.openmemory.enabled` | Enable/disable OpenMemory | `true` |
| `memory.openmemory.serverUrl` | OpenMemory server URL | `"https://api.openmemory.dev"` |
| `memory.openmemory.autoExtractEveryTurns` | Turns between extractions | `5` |
| `memory.openmemory.minImportanceForAutoStore` | Minimum importance to auto-store | `6` |
| `memory.openmemory.forgetThreshold` | Decay score threshold for eviction | `0.3` |
| `memory.openmemory.extractionModel` | LLM for extraction | `"claude-3-5-haiku-20241022"` |
| `memory.openmemory.decayHalfLifeDays` | Decay half-life | `30` |
| `memory.openmemory.maxFactsBeforeEviction` | Max facts before eviction runs | `10000` |
| `memory.openmemory.importMergeStrategy` | Default merge strategy for imports | `"skip_existing"` |
| `memory.openmemory.importDedupThreshold` | Similarity threshold for dedup | `0.85` |
...
1. OpenMemory client library wrapper (TypeScript) with batch store/search + graph support.
...
- NPM package `openclaw-openmemory-plugin`
- Updated OpenMemory client library with LSH support (see separate spec)
