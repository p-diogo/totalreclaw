<!--
Product: OpenMemory
Formerly: tech specs/mcp/TS: OpenMemory MCP Server.md
Version: 1.0
Last updated: 2026-02-24
-->

# Technical Specification: OpenMemory MCP Server
...
This specification describes a **generic MCP (Model Context Protocol) server** that exposes OpenMemory's zero-knowledge encrypted memory capabilities to any MCP-compatible client.
...
By building a generic MCP server, OpenMemory becomes universally accessible.
...
- **openmemory_export**: Export your vault as portable Markdown
...
6. **Rollback**: `import_id` allows `openmemory_forget({ import_id })` within 24h
...
OPENMEMORY_MASTER_PASSWORD=<user's master password>

# Optional
OPENMEMORY_NAMESPACE=default
OPENMEMORY_DEFAULT_IMPORTANCE=5
```
...
// claude_desktop_config.json
{
  "mcpServers": {
    "openmemory": {
      ...
      "env": {
        "OPENMEMORY_SERVER_URL": "http://localhost:8080",
        "OPENMEMORY_MASTER_PASSWORD": "${OPENMEMORY_PASSWORD}"
      }
    }
  }
}
...
See: `TS: OpenMemory Skill for NanoClaw.md` for NanoClaw-specific integration using this MCP server.
...
│   │   └── client/
│   │       └── index.ts        # OpenMemory client wrapper
│   └── README.md
└── README.md                   # Overview
```
...
- [OpenMemory Client Library](/client/)
- [NanoClaw Integration Spec](../nanoclaw/TS: OpenMemory Skill for NanoClaw.md)
