...
# Technical Specification: OpenMemory Server PoC v0.3.1
...
master_password
       │
       ├──► HKDF(pw, salt, "openmemory-auth-v1")  ──► auth_key (for server auth)
       │
       └──► HKDF(pw, salt, "openmemory-enc-v1")   ──► encryption_key (for AES-GCM)
```
...
```
dedup_key   = HKDF-SHA256(master_password, salt, "openmemory-dedup-v1")
...
(`"openmemory-dedup-v1"`) so that it survives encryption key rotation independently.
It follows the same pattern as auth/encryption key separation (§5).

```
master_password
       │
       ├──► HKDF(pw, salt, "openmemory-auth-v1")   ──► auth_key
       ├──► HKDF(pw, salt, "openmemory-enc-v1")    ──► encryption_key
       └──► HKDF(pw, salt, "openmemory-dedup-v1")  ──► dedup_key  (NEW)
```
...
services:
  openmemory-server:
    build: .
    container_name: openmemory-poc
    environment:
      DATABASE_URL: postgresql://openmemory:dev@postgres:5432/openmemory
    ports:
  ...
  postgres:
    ...
    volumes:
      - openmemory-data:/var/lib/postgresql/data
    ports:
