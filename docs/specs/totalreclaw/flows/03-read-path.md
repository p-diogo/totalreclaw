...
4. **Decryption.** Each candidate's `encryptedBlob` is 0x-prefixed hex (`hexBlobToBase64` strips the prefix, hex-decodes, base64-encodes). XChaCha20-Poly1305 decrypts the resulting bytes with the client-side `encryptionKey`. The decrypted JSON is either a canonical Claim (`{t, c, cf, i, sa, ea, e}`) or a legacy `{text, metadata}` doc; `extract_text_from_blob` handles both. The encrypted embedding is decrypted the same way when present.
