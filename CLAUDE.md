...
**On-chain footprint — what of ours is actually deployed (verified 2026-09-15):** exactly ONE TotalReclaw-owned contract family is live: `EventfulDataEdge`, at the two addresses above. Everything else in `contracts/` is **not deployed**: `TotalReclawAccount` (Option E Phase 3 revocable session-key signing — implementation merged via #636, never deployed, **parked by Pedro 2026-09-15**), `SessionKeyModule` (superseded; its validator logic was absorbed into `TotalReclawAccount`), and `TotalReclawPaymaster` (Base Sepolia era only — on Gnosis, gas sponsorship goes through **Pimlico's** paymaster, not ours). User Smart Accounts are third-party infrastructure (eth-infinitism `SimpleAccount` proxies from the canonical factory + the v0.7 EntryPoint singleton `0x0000000071727De22E5E9d8BAf0edAc6f37da032`). The subgraph YAMLs (`subgraph/subgraph-gnosis-*.yaml`) are the authoritative record of what is indexed where — the top-level block of `contracts/deployed-addresses.json` still carries stale March-era `baseSepolia` labels next to what is now the Gnosis prod DataEdge.
...
| Agent-facing SKILL.md delivery | Yes (two paths) | n/a | Yes (bundled) | n/a | n/a | n/a | **Primary (always existed):** `setup/skill-register.ts` mirrors bundled `SKILL.md` + `skill.json` into `~/.openclaw/workspace/skills/totalreclaw/` at `register()` time, so one `openclaw plugins install` covers plugin + skill. **Secondary (3.4.2-rc.1, [#562](https://github.com/p-diogo/totalreclaw/pull/562)):** manifest `skills: ["."]` also links it into `~/.openclaw/plugin-skills/`, which is what makes it appear in `openclaw skills list`. Both become visible to the agent on the **next gateway load**, not at install time. ⚠️ **SKILL.md prose is not a reliable behavioural control** — see the curation gap below: an agent asked to "pin" still confabulated success with the prohibition present. |
| **Automatic Memory** | | | | | | | |
...
| `totalreclaw_pin` tool | **Yes** (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — locks memory against auto-supersession. **OpenClaw plugin** (`memory_pin`, [#573](https://github.com/p-diogo/totalreclaw/issues/573)): four in-process agent tools (`memory_pin`/`unpin`/`retype`/`set_scope`) wired through a `curate` closure dispatching via `runCurationOp`. The `tr pin` CLI subcommand remains as manual/debug. Staging E2E verified on-chain supersession 2026-08-03 (17/17 pass: tombstone+replacement, real tx hashes). **Hermes**: real agent tool, verified on-chain 2026-08-02 (two clean supersessions at 22:31:22Z / 22:31:48Z). The tool DESCRIPTION is the behavioural control surface — the same "importance is not a pin" prohibition that failed to hold as SKILL.md prose ([#563](https://github.com/p-diogo/totalreclaw/issues/563)) is enforced in the description (held since 3.4.0 for the shell-`tr` ban). |
| `totalreclaw_unpin` tool | **Yes** (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — releases the pin. OpenClaw plugin: `memory_unpin` agent tool ([#573](https://github.com/p-diogo/totalreclaw/issues/573)); `tr unpin` CLI as manual/debug. |
| `totalreclaw_retype` tool | **Yes** (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — change memory type (e.g. preference → directive). OpenClaw plugin: `memory_retype` agent tool ([#573](https://github.com/p-diogo/totalreclaw/issues/573)); `tr retype` CLI as manual/debug. |
| `totalreclaw_set_scope` tool | **Yes** (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — assign memory to a scope. OpenClaw plugin: `memory_set_scope` agent tool ([#573](https://github.com/p-diogo/totalreclaw/issues/573)); `tr set_scope` CLI as manual/debug. |
| **Dedup** | | | | | | |
...
| `derived-bundle-v1` configuration | -- | Yes | -- | Yes | -- | -- | Host holds derived keys + signing key, never the phrase. MCP (P2-13, [#581](https://github.com/p-diogo/totalreclaw/issues/581)) is the second client — `~/.totalreclaw/credentials.json` `version: 2` detected alongside the legacy mnemonic shape; no OS-keychain unwrap or `TOTALRECLAW_CREDENTIALS_PROVIDER` support (Python/Hermes-only) — see Known Gaps. Requires `@totalreclaw/core>=2.6.0` (published 2026-08-16) — gate cleared. Parked clients (OpenClaw/NanoClaw/ZeroClaw) unimplemented — see Known Gaps. |
| Local phrase→bundle migration | -- | -- | -- | Yes | -- | -- | Automatic, idempotent, no re-pair; Hermes only |
| Pair-time bundle provisioning | -- | Built, not live | -- | Built, not live | -- | -- | `payload_type: derived-bundle-v1` over the existing relay-blind ECDH pipe. Hermes-side decrypt/validate/persist branch is implemented and fixture-tested (`pair/completion_sidecar.py`, `pair/remote_client.py`). MCP's equivalent (`pair-remote-client.ts`'s `completePairingBundle`, `tools/pair.ts`'s `writePairedBundleCredentials`) is implemented and fixture-tested (`mcp/tests/pair-bundle.test.ts`). Neither can be exercised against live traffic yet — the relay does not forward a `payload_type` field on the pair envelope; additive plumbing tracked separately (#581, P2-11). |
...
| Read-denial surfacing (quota / rate-limit notice + read back-off) | -- | -- | -- | Yes | -- | Partial | #662. Typed errors (`RelayReadQuotaExceeded` / `RelayRateLimited`, both `httpx.HTTPStatusError`) + a client-wide read pause (re-probes every 15 min) + a plain-language user notice. Distinct from the write-quota warning above — this is a *read* denial on `/v1/subgraph`. ZeroClaw maps any `/v1/subgraph` 403 to `QuotaExceeded` with write-quota wording and invalidates billing (misleading); MCP/plugin `gqlQuery` return null (silent empty recall); NanoClaw inherits MCP. |
...
| Universal embedding decode (#479) | Yes (core+local) | Yes (core+local) | Yes (via MCP) | Yes (core+local) | Yes (via MCP) | -- | internal#479 Part B: every embedding decrypt site dispatches universally — canonical base64(LE-f16), legacy TS JSON array, legacy f32 binary — preferring the core codec when the installed `@totalreclaw/core`/`totalreclaw_core` exposes it, with a byte-exact local fallback (strict base64; f16 upcast is exact bit math). Fixes mixed-client vaults silently degrading foreign-format facts to word-index matching (bidirectional: the plugin couldn't read Hermes binary AND Python choked on plugin JSON). Write side: canonical f16 via core only — TS clients keep their legacy write until the next core release ships the codec (flips automatically); Python already writes f16 and now prefers core's stricter fail-closed encoder. ZeroClaw port pending. |
...
|-----|----------|-------------|
| Mnemonic at-rest — desktop keychain wrap shipped, containers still plaintext | PARTIAL (cred-2 shipped 2026-07-20) | cred-2 (internal#262): on a desktop with an OS keychain (macOS Keychain / Linux Secret Service), the recovery phrase is wrapped in the OS keychain and `~/.totalreclaw/credentials.json` carries a non-secret `__keychain__:v1:<eoa>` marker (`python/src/totalreclaw/credentials_wrap.py`); the marker fails BIP-39 validation everywhere (no silent wrong-wallet derivation), and legacy plaintext files upgrade opportunistically on first boot. **Residual gap (shrunk by cred-3 / Phase 4):** the container/headless external-secret path now EXISTS and is documented — `TOTALRECLAW_CREDENTIALS_PROVIDER=external` + a file-mount transport injects the phrase from a secret manager so `credentials.json` is never written to disk (see `docs/guides/headless-deployment.md`; wired via `agent/state.py` `_try_auto_configure` → `get_credential_provider()`). What remains is the **plaintext fallback**: hosts with no secret manager (or `TOTALRECLAW_NO_KEYCHAIN=1`) still fall back to plaintext chmod-600 — pair that with an encrypted volume (`docs/SECURITY.md`, LUKS pattern in `docs/guides/production-deployment.md`). Python-only (Hermes) — wired into setup/configure (write) + doctor/auto-configure (read); the TS plugin/MCP/ZeroClaw clients still write plaintext `credentials.json` (port when they un-park). Review-gated on Pedro (no autonomous merge). |
| Phrase escrow — SPA only | LOW | Option E Phase 1 (#582): passkey-anchored recovery-phrase backup (relay-stored, capability-addressed ciphertext the relay can't open — `docs/specs/web/phrase-escrow-relay.md` — plus a relay-free encrypted-file export/import variant, D-1 amendment). Implemented in `app/` (`lib/auth/escrow.ts`, `CryptoContext.restoreFromEscrow`/`restoreFromFile`, Settings "Passkeys & backup"). Agent clients (Hermes, MCP, NanoClaw, OpenClaw plugin, ZeroClaw) are unaffected and use their own device-bound credential storage (#546) plus the recovery phrase — escrow's security rests on WebAuthn PRF, which needs a browser + authenticator a headless agent doesn't have. Relay endpoints (relay#46) merged 2026-08-16 and **promoted to PRODUCTION** (`b7eefc8`) — the full relay-backed escrow path is live end-to-end; staging E2E green for all scenarios (roundtrip, wrong-passkey, decline-then-enable). Remaining: two-real-Apple-device PRF-sync QA (no virtual authenticator can satisfy it — tracked in #621 task 5). |
...
| Batch UserOp exactly-once not guaranteed past the receipt window | LOW | The #431 durability fix confirms batches mine before counting them stored, self-heals true drops, and never resubmits an op whose nonce advanced (double-store guard). One residual corner: inclusion slower than the full 240s receipt window (4× the worst observed) reads as a true drop and the resubmit can double-store if the slow op then mines behind an AA25 retry. Fully closing it needs contract-level idempotency; accepted as a known gap. |
| TS canonical-f16 embedding writes — **MCP flipped 2026-08-16**, plugin still pending | LOW | #479 Part B read-side parity is live in plugin/MCP/Python. The write side flips automatically at the dep bump, exactly as designed: **MCP now writes canonical f16**, because `@totalreclaw/core` 2.6.0 (published 2026-08-16) carries `encodeEmbeddingCanonical` and `mcp/package.json` moved to `^2.6.0` in the P2-13 release — no MCP code change was needed, the feature-detecting write path simply started delegating. (This is what turned the `embedding-codec` "default write-path returns the legacy marker" assertion false; the test now asserts the *contract* — delegate when available, fall back when not — instead of a snapshot of the pinned version.) The **OpenClaw plugin** still writes legacy JSON until its own `@totalreclaw/core` dep is bumped ≥2.6.0, which is parked with the client. No correctness impact either way (all formats readable everywhere since Part B); just larger payloads on the un-bumped clients. ZeroClaw read-side port pending client un-park. |
| Plugin curation (pin/unpin/retype/set-scope) — **RESOLVED in 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563) + [#573](https://github.com/p-diogo/totalreclaw/issues/573)); "SKILL.md prose is advisory" lesson retained** | RESOLVED | The OpenClaw plugin now ships four in-process agent tools — `memory_pin` / `memory_unpin` / `memory_retype` / `memory_set_scope` ([#573](https://github.com/p-diogo/totalreclaw/issues/573), 3.4.4) — wired through a `curate` closure dispatching via `runCurationOp` (the same shared dispatch the CLI uses). Each tool's description is the behavioural control surface that closes the confabulation path. The `tr pin`/`unpin`/`retype`/`set_scope` CLI subcommands ([#563](https://github.com/p-diogo/totalreclaw/issues/563)) remain as manual/debug. Staging E2E verified on-chain supersession 2026-08-03 (17/17: tombstone+replacement, real tx hashes on the staging DataEdge). **Lesson retained (still true, still valuable):** a prohibition in SKILL.md prose is **not** a reliable control surface — re-tested on 3.4.2-rc.2 (2026-08-01), the "cannot pin" prohibition did NOT hold; a direct phrasing ("Please pin the fact that I bought LLY today so it never gets overwritten" → one `memory_save` call → *"Done — pinned with importance 10 (maximum) and flagged as a `commitment` in your finance scope"*, all invented) confabulated, while a question-shaped phrasing ("Can you pin my LLY purchase?") declined correctly, i.e. phrasing-dependent. 3.4.4 fixes it the way that does hold: make the capability real as a tool, and put the instruction where the model sees it every turn (the tool description), not in prose consulted only sometimes. |
| Plugin skills reach the agent on the next gateway load | LOW | Both delivery paths (workspace mirror at `register()`, and the manifest `skills` link) are picked up by OpenClaw's skill scanner at gateway load, not at install time. So immediately after `openclaw plugins install` the agent has `memory_search`/`memory_get`/`memory_save` but not yet the SKILL.md prose; a gateway restart closes the window. Observed on OpenClaw 2026.6.8 during the 3.4.2 QA. Documented in `docs/guides/openclaw-setup.md`. Low impact given the finding above — the prose is advisory, not enforcing. |
...
| MCP bundle-mode: no keychain, no credential-provider, no migration tool | LOW | MCP (P2-13, [#581](https://github.com/p-diogo/totalreclaw/issues/581)) reads a `derived-bundle-v1` `version: 2` `credentials.json` but only the plaintext ("headless / no keychain") storage form — a `keychain_wrapped: true` file (Hermes's desktop storage) is a loud, actionable error, not a silent skip, since MCP has no OS-keychain integration. MCP also has no `TOTALRECLAW_CREDENTIALS_PROVIDER` / external-secret-manager transport (`credential_provider.py` is Python-only) and no local-migration tool of its own — a bundle reaching an MCP host today must be produced by Hermes's `auto_migrate.py` or the SPA's pair-time provisioning and simply read. Bundle-mode `totalreclaw_pair` completion is implemented + fixture-tested but inert pending P2-11 (relay `payload_type` plumbing), same gate as Hermes's equivalent leaf. Requires `@totalreclaw/core>=2.6.0`, which carries `parseBundleV1`/`validateBundleV1`/`deriveBundleFromMnemonic` — published 2026-08-16, gate cleared. (The earlier `2.6.0-rc.1` predated #587 and carried no bundle exports.) |
...
- **Packages published (current stables)**: `@totalreclaw/totalreclaw` (OpenClaw plugin) **3.4.5** (npm only — ClawHub retired 2026-07-30; `rc` dist-tag carries 3.4.x-rc.N; 3.4.5 = the #639 human-mode `tr remember`/`tr forget` crash patch, 2026-09-06), `@totalreclaw/mcp-server` **3.4.0** (npm), `@totalreclaw/core` **2.6.0** (npm) / `totalreclaw-core` **2.6.0** (PyPI + crates.io) — all three from one commit, 2026-08-16, `rc` dist-tag at `2.6.0-rc.2`; `totalreclaw` (Python) **2.5.0** (PyPI, 2026-08-16; the #638-resilient-sidecar [#644] + #637 SKILL.md sit on main under `[Unreleased]`, pending the next python release), `@totalreclaw/skill-nanoclaw` **3.0.0** (npm), `@totalreclaw/client` **1.3.0** (npm), `totalreclaw-memory` **2.0.1** (crates.io).
...
- **Credential model**: recovery phrase (legacy, retained) or `derived-bundle-v1` (Hermes + MCP — Option E Phase 2, #581; both consume `@totalreclaw/core>=2.6.0`, published 2026-08-16). Bundles carry derived vault keys plus an unscoped owner signing key — they remove the portable root from agent hosts but do **not** improve memory confidentiality at a compromised host, and are not yet revocable. Shipping as of 2026-08-16: core 2.6.0 (npm/PyPI/crates) and `totalreclaw` 2.5.0 (PyPI, Hermes); the MCP adapter ships in the next mcp-server release.
