...
| `totalreclaw_pin` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — locks memory against auto-supersession. **OpenClaw plugin: no agent *tool*, but the `tr pin` CLI subcommand shipped in 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563))** — invoked via `node "$TR_CLI" pin <id>`. The `memory_save` tool description points at it and forbids faking a pin with a store. (3.4.2 wrongly advertised a pin that didn't exist; the 3.4.2-rc.2 SKILL.md prohibition didn't hold — see the resolved Known Gap.) |
| `totalreclaw_unpin` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — releases the pin. Same as `totalreclaw_pin` in the plugin: no agent tool, `tr unpin` CLI shipped 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| `totalreclaw_retype` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — change memory type (e.g. preference → directive). Same as `totalreclaw_pin` in the plugin: no agent tool, `tr retype` CLI shipped 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| `totalreclaw_set_scope` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — assign memory to a scope. Same as `totalreclaw_pin` in the plugin: no agent tool, `tr set_scope` CLI shipped 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| **Dedup** | | | | | | |
