...
| Update-available notice | -- | -- | -- | Yes | -- | -- | Hermes-only. Relay serves `features.latest_stable_python` (set via `LATEST_STABLE_PYTHON` env — dark until configured); client compares vs installed `__version__` (rc<final aware), nudges once/24h via quota-warning channel. Kill-switch `TOTALRECLAW_DISABLE_UPDATE_NOTICE=1`. Fires fleet-wide on the one env flip at each stable promote. **Hardened post-#486**: `totalreclaw_status` now surfaces `installed_version`/`latest_stable`/`update_available` on demand, with a best-effort PyPI JSON fallback (3s timeout, 24h cache, same kill-switch) when the relay value is dark — used only in status/doctor, never per-turn hooks. `totalreclaw doctor` warns when the installed build is an RC older than the current stable (staging-baked RC on a production bot). |
| Recall-health fail-loud notice | -- | -- | -- | Yes | -- | -- | internal#486 follow-up. When auto-recall raises OR returns 0 results while the billing cache shows `free_writes_used > 0`, the shared `hooks.recall_for_query` (drives both `pre_llm_call` and `MemoryProvider.prefetch`) injects a `[totalreclaw]` memory-health warning instructing the agent to tell the user recall is unavailable — instead of a silent None that looks like "no memories". Rate-limited once/24h (`recall_health.py` sentinel); kill-switch `TOTALRECLAW_DISABLE_RECALL_HEALTH_NOTICE=1`. Companion diagnostic: `totalreclaw doctor --recall-smoke` runs a read-only end-to-end recall and FAILs on the writes>0-but-0-candidates signature. Hermes-only (other clients' recall drivers pending un-park). |
