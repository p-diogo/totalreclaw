...
| Re-crystallize / re-key backfill (collapsed sessions) | DESIGN ONLY | Design + scaffold for a client-side backfill that re-segments a vault whose on-chain `session_id`s collapsed (pre-#429/#434 write-side bug) into coherent sessions + fresh Crystals, tombstoning the old. Spec: `docs/specs/totalreclaw/recrystallize-backfill.md`. Scaffold: `python/src/totalreclaw/recrystallize.py` — dry-run planner + quota estimator (`2·F + S_multi + C_old`) implemented & unit-tested; on-chain write/tombstone path is a stubbed `NotImplementedError`. Must run AFTER the write-side fix is live for the target client. Not built. |
