...
| `derived-bundle-v1` configuration | -- | Yes (unreleased) | -- | Yes | -- | -- | Host holds derived keys + signing key, never the phrase. MCP (P2-13, [#581](https://github.com/p-diogo/totalreclaw/issues/581)) is the second client — `~/.totalreclaw/credentials.json` `version: 2` detected alongside the legacy mnemonic shape; no OS-keychain unwrap or `TOTALRECLAW_CREDENTIALS_PROVIDER` support (Python/Hermes-only) — see Known Gaps. Release-gated on an `@totalreclaw/core` publish carrying the bundle WASM bindings. Parked clients (OpenClaw/NanoClaw/ZeroClaw) unimplemented — see Known Gaps. |
...
| TS canonical-f16 embedding writes pending core release | LOW | #479 Part B read-side parity is live in plugin/MCP/Python; TS clients still WRITE their legacy formats (plugin JSON, MCP f32) until a published `@totalreclaw/core` carries `encodeEmbeddingCanonical` — the wiring feature-detects and flips to canonical automatically at that dep bump. No correctness impact (all formats readable everywhere now); just larger payloads until then. ZeroClaw read-side port pending client un-park. |
...
- **Credential model**: recovery phrase (legacy, retained) or `derived-bundle-v1` (Hermes + MCP as of this writing — Option E Phase 2, #581; MCP's P2-13 adapter is unreleased, release-gated on an `@totalreclaw/core` publish carrying the bundle WASM bindings). Bundles carry derived vault keys plus an unscoped owner signing key — they remove the portable root from agent hosts but do **not** improve memory confidentiality at a compromised host, and are not yet revocable. Not yet published to any registry.
