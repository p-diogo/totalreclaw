...
| Single-chain (Gnosis-only) policy | **IN PROGRESS** | **NOT shipped.** Dual-chain is live: Free → Base Sepolia (84532), Pro → Gnosis (100) — verified 2026-05-30 against Railway (`PIMLICO_CHAIN_ID=84532`, `PRO_PIMLICO_CHAIN_ID=100`, both envs). Migration tracked as ops-1 (`totalreclaw-internal#283`, OPEN P0). The legacy cross-chain `migrate` tool was retired, but per-tier chain routing remains. |
