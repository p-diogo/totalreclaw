...
| Agent-facing SKILL.md delivery | Yes (self-published) | n/a | Yes (bundled) | n/a | n/a | n/a | Plugin manifest declares `skills: ["."]`, so OpenClaw links its bundled SKILL.md into `~/.openclaw/plugin-skills/` (shipped 3.4.2-rc.1, [#562](https://github.com/p-diogo/totalreclaw/pull/562)). Previously the prose only arrived via a separate `openclaw skills install` from ClawHub; after that channel was retired ([#561](https://github.com/p-diogo/totalreclaw/pull/561)) an npm-only install had to be self-sufficient. **Publishes at gateway START, not at install** — the post-install in-process reload re-registers tools but not skills. |
| **Automatic Memory** | | | | | | | |
...
| Plugin curation (pin/unpin/retype/set-scope) missing | MEDIUM | The OpenClaw plugin has no pin tool and no `tr pin`/`unpin`/`retype`/`set_scope` CLI command — they exist only in MCP + NanoClaw. Until 3.4.2-rc.2, `SKILL.md` **advertised them anyway**, so an agent asked to "pin that" stored an ordinary memory and reported a successful pin — a confident success report for an operation that never happened, the same class as the #551 shell-`tr` data loss. Caught in the 3.4.2-rc.1 fresh-install QA. rc.2 makes SKILL.md tell the truth ("this client cannot pin"); implementing the commands for real is tracked in [#563](https://github.com/p-diogo/totalreclaw/issues/563). The only curation the plugin supports is `tr forget <factId>`. |
| Plugin skills publish only at gateway start | LOW | OpenClaw resolves plugin-published skills during config load. The in-process reload after `openclaw plugins install` re-registers tools but does **not** re-run the skill publisher, so a freshly-installed plugin has `memory_search`/`memory_get`/`memory_save` but no SKILL.md guidance until the gateway restarts. Observed live on OpenClaw 2026.6.8 during the 3.4.2-rc.1 QA. Documented in `docs/guides/openclaw-setup.md`; a plugin-side fix would need OpenClaw to expose a skill-republish hook. |
