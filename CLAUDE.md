...
| `totalreclaw_pin` tool | **No** (see #563) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — locks memory against auto-supersession. **Not in the OpenClaw plugin**: no pin tool and no `tr pin` CLI command. SKILL.md wrongly advertised one until 3.4.2-rc.2, which made the agent claim successful pins that never happened ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| `totalreclaw_unpin` tool | **No** (see #563) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1. Same plugin gap as `totalreclaw_pin`. |
| `totalreclaw_retype` tool | **No** (see #563) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — change memory type (e.g. preference → directive). Same plugin gap as `totalreclaw_pin`. |
| `totalreclaw_set_scope` tool | **No** (see #563) | Yes | Yes (via MCP) | -- | Yes (via MCP) | -- | New in v1 — assign memory to a scope. Same plugin gap as `totalreclaw_pin`. |
| **Dedup** | | | | | | |
...
| Plugin curation (pin/unpin/retype/set-scope) missing — **and SKILL.md cannot stop the agent faking it** | MEDIUM | The OpenClaw plugin has no pin tool and no `tr pin`/`unpin`/`retype`/`set_scope` CLI command — they exist only in MCP + NanoClaw. Until 3.4.2-rc.2 `SKILL.md` **advertised them anyway**, so an agent asked to "pin that" stored an ordinary memory and reported success — same class as the #551 shell-`tr` data loss. rc.2 replaced that with an explicit prohibition. **Re-tested on rc.2 (2026-08-01): the prohibition DID NOT hold.** "Please pin the fact that I bought LLY today so it never gets overwritten" → one `memory_save` call → *"Done — pinned with importance 10 (maximum) and flagged as a `commitment` in your finance scope"*, all invented. A softer phrasing ("Can you pin my LLY purchase?") declined correctly, so the behaviour is phrasing-dependent. **Lesson: a prohibition in SKILL.md is not a reliable control surface** — the constraint has to live where the model always sees it (the `memory_save` tool description) or be enforced in code. Tracked in [#563](https://github.com/p-diogo/totalreclaw/issues/563). The only curation the plugin supports is `tr forget <factId>`. |
