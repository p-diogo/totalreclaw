...
| `totalreclaw_pin` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — locks memory against auto-supersession. **Hermes has it as a real agent tool and it works** — verified on-chain 2026-08-02 (two clean supersessions, tombstone+replacement, txs at 22:31:22Z / 22:31:48Z). The matrix previously showed `--` for Hermes, which was wrong. **The plugin is the outlier**: CLI-only shell-out, no agent tool. **OpenClaw plugin: no agent *tool*, but the `tr pin` CLI subcommand shipped in 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563))** — invoked via `node "$TR_CLI" pin <id>`. The `memory_save` tool description points at it and forbids faking a pin with a store. (3.4.2 wrongly advertised a pin that didn't exist; the 3.4.2-rc.2 SKILL.md prohibition didn't hold — see the resolved Known Gap.) |
| `totalreclaw_unpin` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — releases the pin. Same as `totalreclaw_pin` in the plugin: no agent tool, `tr unpin` CLI shipped 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| `totalreclaw_retype` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — change memory type (e.g. preference → directive). Same as `totalreclaw_pin` in the plugin: no agent tool, `tr retype` CLI shipped 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| `totalreclaw_set_scope` tool | CLI only (3.4.4) | Yes | Yes (via MCP) | **Yes** | Yes (via MCP) | -- | New in v1 — assign memory to a scope. Same as `totalreclaw_pin` in the plugin: no agent tool, `tr set_scope` CLI shipped 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)). |
| **Dedup** | | | | | | |
...
| Plugin curation (pin/unpin/retype/set-scope) — **RESOLVED in 3.4.4 ([#563](https://github.com/p-diogo/totalreclaw/issues/563)); "SKILL.md prose is advisory" lesson retained** | RESOLVED | The OpenClaw plugin now ships `tr pin` / `tr unpin` / `tr retype` / `tr set_scope` CLI subcommands (3.4.4, [#563](https://github.com/p-diogo/totalreclaw/issues/563)) that delegate to the same `executePinOperation` / `executeRetype` / `executeSetScope` the MCP server and NanoClaw already used — so an agent told to "pin that" finally has a real path instead of confabulating a `memory_save` and reporting a successful pin. There is still **no agent-facing pin *tool*** (the plugin's agent tools remain `memory_search` / `memory_get` / `memory_save`); curation runs via `node "$TR_CLI" <cmd>`, and the `memory_save` tool description points at it. **Lesson retained (still true, still valuable):** a prohibition in SKILL.md prose is **not** a reliable control surface — re-tested on 3.4.2-rc.2 (2026-08-01), the "cannot pin" prohibition did NOT hold; a direct phrasing ("Please pin the fact that I bought LLY today so it never gets overwritten" → one `memory_save` call → *"Done — pinned with importance 10 (maximum) and flagged as a `commitment` in your finance scope"*, all invented) confabulated, while a question-shaped phrasing ("Can you pin my LLY purchase?") declined correctly, i.e. phrasing-dependent. 3.4.4 fixes it the way that does hold: make the capability real in code, and put the instruction where the model sees it every turn (the tool description), not in prose consulted only sometimes. |
