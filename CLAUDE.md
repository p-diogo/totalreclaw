...
| Keychain-wrapped credentials.json breaks co-installed clients' reads | MEDIUM (documented) | All clients share `~/.totalreclaw/credentials.json`. Once Hermes wraps the phrase (#262 marker scheme), a co-installed OpenClaw plugin reading the same file hits an unhandled rejection on the marker (hard fail — tracked for a graceful error), and the MCP server degrades to unconfigured. The marker fails BIP-39 loudly everywhere (verified incl. the Rust core), so no client can ever derive a wrong wallet — the break is availability, not safety. Workaround: `TOTALRECLAW_NO_KEYCHAIN=1` on shared-client machines. Other clients still write plaintext (their wrap ports ride the cred track). |
