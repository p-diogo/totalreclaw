/**
 * NanoClaw Agent Runner — TotalReclaw Overlay
 *
 * This is a modified copy of the original agent-runner index.ts with two changes:
 *   1. Added 'mcp__totalreclaw__*' to allowedTools
 *   2. Added 'totalreclaw' MCP server to mcpServers config
 *
 * Original: nanoclaw/container/agent-runner/src/index.ts
 *
 * Input protocol:
 *   Stdin: Full ContainerInput JSON (read until EOF, like before)
 *   IPC:   Follow-up messages written as JSON files to $WORKSPACE_DIR/ipc/input/
 *          Files: {type:"message", text:"..."}.json — polled and consumed
 *          Sentinel: $WORKSPACE_DIR/ipc/input/_close — signals session end
 *   Env:   WORKSPACE_DIR overrides the default /workspace root (for E2E testing)
 *
 * Stdout protocol:
  ...
  for await (const message of query({
    ...
    options: {
      ...
      mcpServers: {
        ...
        totalreclaw: {
          command: process.env.TOTALRECLAW_MCP_PATH ? 'node' : 'npx',
          args: process.env.TOTALRECLAW_MCP_PATH
